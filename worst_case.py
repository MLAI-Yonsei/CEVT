...
if True: #worstcase eval code
    
    class WorstCaseDataset(Dataset):
        def __init__(self, original_dataset, worst_case_indices):
            self.original_dataset = original_dataset
            self.worst_case_indices = worst_case_indices

        def __len__(self):
            return len(self.worst_case_indices)

        def __getitem__(self, idx):
            return self.original_dataset[self.worst_case_indices[idx]]

        def __getattr__(self, name):
            return getattr(self.original_dataset, name)

    def worst_dataloader(dataloader):
        all_diff_tensors = []
        all_indices = []

        # 데이터를 순회하며 diff_tensor와 인덱스 수집
        for i, (*features, diff_tensor, treatment) in enumerate(dataloader):
            all_diff_tensors.append(diff_tensor)
            all_indices.extend(range(i * dataloader.batch_size, i * dataloader.batch_size + len(diff_tensor)))
        ...
        # 데이터를 다시 순회하며 분류
        for i, diff_value in enumerate(all_diff_tensors):
            key = int(max(diff_value.unique()))
            grouped_indices[key].append(all_indices[i])

        # worst case (key=0)에 해당하는 인덱스들로 새로운 데이터셋 생성
        worst_case_dataset = WorstCaseDataset(dataloader.dataset.dataset, grouped_indices[0])
        
        return DataLoader(worst_case_dataset, batch_size=len(worst_case_dataset))

    val_dataloader = worst_dataloader(val_dataloader)
    test_dataloader = worst_dataloader(test_dataloader)
...
negative_acc_y_t1, negative_acc_d_t1, ce_y_t1, ce_d_t1 = utils.CE_worstcase(args, model, val_dataloader, 't1')
negative_acc_y_t2, negative_acc_d_t2, ce_y_t2, ce_d_t2 = utils.CE_worstcase(args, model, val_dataloader, 't2')

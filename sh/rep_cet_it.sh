...
do
for drop_out in 0 0.1 0.2 
do
CUDA_VISIBLE_DEVICES=${GPU_IDS[$IDX]}  /mlainas/bubble3jh/anaconda3/envs/cluster/bin/python3 run_itransformer.py \
--model=iTransformer --use_treatment --single_treatment --run_group=${run_group} \
--hidden_dim=${hidden_dim} --num_layers=${num_layers} --num_heads=${num_heads} \
--lr_init=${lr_init} --optim=adam --wd=${wd} --drop_out=${drop_out} --seed=${seed} &

...
import pylab as plt
from utils.utils_syn import IsingModel, factor2Df, sampling
...
class IsingDataSimulator:
    ...
    def get_samples(self, n, seed=None):
        ...
        # Ensure samp_pos and samp_neg are not None
        if samp_pos is None:
            samp_pos = pd.DataFrame(columns=[str(i) for i in range(p + 1)])
        if samp_neg is None:
            samp_neg = pd.DataFrame(columns=[str(i) for i in range(p + 1)])
...
class BayesNetDataSimulator:
    ...
    def __init__(self, causal_effect=False, q=0, valid_z=True):
        ...
        self.bn = BayesianNetwork(edge_list)
    ...
    def get_samples(self, iv_samp, seed=None):
        ...
        bn_samp = pd.concat([
            sampling(world, world["z"] == 1, n=Counter(iv_samp["z"])[1], random_state=rng).set_index(
                iv_samp[iv_samp["z"] == 1].index),
            sampling(world, world["z"] == 0, n=Counter(iv_samp["z"])[0], random_state=rng).set_index(
        ...
        for i in range(self.q):
            ...
            invalid_samp = pd.concat([
                sampling(world, world["confounder"] == 1, n=Counter(bn_samp["confounder"])[1], random_state=rng).set_index(
                    bn_samp[bn_samp["confounder"] == 1].index),
                sampling(world, world["confounder"] == 0, n=Counter(bn_samp["confounder"])[0], random_state=rng).set_index(
